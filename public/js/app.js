...
window.onload = function() {
    ...
    window.addEventListener('scroll', function() {
        if($toc){
            ...
            if(getScrollTop() <= top){ $toc.style ; } else { $toc.style.position="fixed" $toc.style.top="5px" $toc.style.left="left" + "px"; $toc.style.width="width" "px" cleartimeout(timer); timer="setTimeout(function" fn() scrollcallback(); }, 200); }); 返回顶部 $backtotop.onclick="function()" cancelanimationframe(timer); var stop="getScrollTop();" if (stop> 0) {
